...
const services = [
  {
    ...
    bgClass: "bg-white text-black hover:bg-[#00295A] hover:text-white",
  },
  {
    id: 2,
    ...
    <div className="w-full px-4 md:px-6 lg:pt-20 lg:px-36 max-w-8xl mx-auto bg-white">
      <div className="hidden lg:grid grid-cols-3 gap-7 2xl:gap-10 3xl:gap-14">
        {services.map((service, index) => (
          <div
            key={service.id}
            className={`${service.bgClass} cursor-pointer p-5 3xl:p-10 rounded-xl shadow-corner hover:shadow-none transition-all duration-200 ease-linear`}
          >
            <div className="px-3">
              <Image
                width={40}
            ...
            <p className="text-base lg:text-lg 2xl:text-2xl 3xl:text-3xl 2xl:font-light">
              {service.description}
            </p>
          </div>
        ))}
