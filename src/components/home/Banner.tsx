  ...
  return (
    <div className="pt-20 relative text-white h-screen md:h-auto lg:min-h-screen lg:pt-48 2xl:pt-56 flex flex-col justify-center items-center">
      ...
      <div className="relative z-10 text-center px-4 md:px-6 lg:px-20 md:py-20 lg:py-0">
        <h1 className="text-3xl md:text-4xl lg:text-5xl 2xl:text-7xl font-semibold mx-auto mb-5">
          Web Design & Development
          <br /> Digital marketing & Mobile App&apos;s
        </h1>
        <p className="w-4/5 md:w-2/3 text-sm md:text-base 2xl:text-xl mx-auto font-light mb-6">
        ...
        <div className="flex flex-wrap pt-10 lg:pb-20 justify-center items-center">
          {images.map((image, index) => {
            return (
              <div className="md:w-[10%] lg:w-[16%] px-3 lg:px-0 py-3 grayscale" key={index}>
                <Image
                  priority
                  height={100}
                  src={image?.url}
                  width={image?.width}
                  alt={`image-${index}`}
                  className={`h-fit w-10 md:w-20 lg:w-auto 2xl:w-28 ${
