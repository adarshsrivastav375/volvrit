...
const PortfolioOffer = () => {
  ...
  return (
    <>
      <div className="max-w-9xl mx-auto pb-5 md:py-8 text-center px-4 md:px-6 lg:px-20">
      ...
      <CardComponent />
      <div className="max-w-9xl mx-auto pt-8 text-center px-4 md:px-6 lg:px-20">
        <h4 className="text-4xl font-semibold 2xl:text-5xl 3xl:text-6xl 4xl:text-7xl mb-5">
          The Portfolio Offers:
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-5 lg:gap-10 py-10">
          {portfolioOffers.map((item) => {
            return (
              <div
                key={item?.id}
                className="flex bg-[#EAEEF2] flex-col rounded-2xl p-5 md:p-8 justify-start items-center"
              >
                <Image
                  src={item?.imageUrl}
                  alt={item?.title}
                ...
                />
                <h4 className="text-2xl md:text-xl my-3 font-bold leading-6 w-full lg:w-2/3 text-gray-800">
                  {item?.title}
                </h4>
